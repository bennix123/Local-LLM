...
from .services import txn_store
import pymupdf
...
async def upload_document(file: UploadFile = File(...), user_id: str = Depends(get_current_user)):
    ...
    try:
        ...
        # ---- Transaction statement fast-path (deterministic SQL layer) ----
        # Statements with thousands of rows can't be answered by RAG (summing the
        # top-k chunks is wrong) and would also choke the per-table Camelot+LLM
        # ingest. Detect them up front, parse rows into SQLite, and skip RAG.
        doc = pymupdf.open(temp_path)
        head_text = "".join(doc[i].get_text("text") for i in range(min(3, len(doc))))
        n_pages = len(doc)
        doc.close()

        if txn_store.is_transaction_statement(head_text):
            rows = txn_store.ingest_pdf(temp_path, file.filename, user_id)
            os.remove(temp_path)
            print(f"✓ Transaction statement ingested: {file.filename} ({rows:,} rows, user: {user_id})")
            return UploadResponse(
                filename=file.filename,
                collection_name=f"txn::{file.filename}",
                pages=n_pages,
                total_docs=rows,
                message=f"Processed {rows:,} transactions from {file.filename} (deterministic SQL mode)",
            )
...
async def query_documents(request: QueryRequest, user_id: str = Depends(get_current_user)):
    ...
    try:
        # ---- Deterministic SQL layer first (exact numbers, no hallucination) ----
        doc_name = request.document_names[0] if request.document_names and len(request.document_names) == 1 else None
        sql_answer = txn_store.answer(request.question, user_id, doc_name)
        if sql_answer is not None:
            return QueryResponse(
                answer=sql_answer,
                sources=[],
                question=request.question,
                searched_docs=request.document_names or [],
            )
...
async def query_documents_stream(request: QueryRequest, user_id: str = Depends(get_current_user)):
    """
    ...
    async def generate():
        ...
        # Deterministic SQL layer first (exact numbers stream as one block)
        doc_name = request.document_names[0] if request.document_names and len(request.document_names) == 1 else None
        sql_answer = txn_store.answer(request.question, user_id, doc_name)
        if sql_answer is not None:
            yield f"data: {json.dumps({'type': 'token', 'content': sql_answer})}\n\n"
        ...
        # Retrieve chunks
        if len(doc_names) == 1:
            chunks = engine.retrieve_single_document(doc_names[0], request.question, user_id, request.n_results)
        else:
            chunks = engine.retrieve_multiple_documents(doc_names, request.question, user_id, request.n_results)
