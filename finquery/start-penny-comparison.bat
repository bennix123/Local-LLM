...
cd /d "%~dp0"
echo ============================================================
echo    Penny  -  Starting Both 8B and 4B Servers for Comparison
echo ============================================================
...
start "Penny 8B Server" cmd /c _penny_server.bat

REM --- start the 4B server (port 5668) ---
echo Starting 4B server (Qwen 3B/4B) on port 5668...
start "Penny 4B Server" cmd /c _penny_server_4b.bat
...
REM --- wait until 4B server answers on :5668 ---
echo Waiting for 4B server (port 5668) to come up...
:wait4b
timeout /t 1 /nobreak >nul
curl -s -o nul http://127.0.0.1:5668/status
if errorlevel 1 goto wait4b
echo 4B Server is up.

echo.
echo ============================================================
echo    Comparison is ready!
echo.
echo    Llama 8B (Port 5667):  http://127.0.0.1:5667/app
echo    Qwen 3B/4B (Port 5668): http://127.0.0.1:5668/app
echo.
